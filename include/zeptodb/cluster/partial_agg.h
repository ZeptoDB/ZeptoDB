...
inline MergeStrategy detect_merge_strategy(
    const std::vector<zeptodb::sql::QueryResultSet>& results)
{
...
// ============================================================================
inline zeptodb::sql::QueryResultSet merge_scalar_results(
    const std::vector<zeptodb::sql::QueryResultSet>& results)
{
    if (results.empty()) return {};

    // Find a non-error result to use as template
    const zeptodb::sql::QueryResultSet* tmpl = nullptr;
    for (const auto& r : results) {
    ...
    for (const auto& r : results) {
        ...
        for (size_t ci = 0; ci < ncols && ci < src.size(); ++ci) {
            ...
            switch (detect_agg(merged.column_names[ci])) {
                ...
                case AggFunc::VWAP: {
                    int64_t tmp;
                    if (__builtin_add_overflow(row[ci], v, &tmp)) {
                        merged.error = "integer overflow in distributed SUM/VWAP";
                        return merged;
                    }
                    row[ci] = tmp;
                    break;
...
// ============================================================================
inline zeptodb::sql::QueryResultSet merge_concat_results(
    const std::vector<zeptodb::sql::QueryResultSet>& results)
{
    if (results.empty()) return {};
...
// ============================================================================
inline zeptodb::sql::QueryResultSet merge_results(
    const std::vector<zeptodb::sql::QueryResultSet>& results,
...
// ============================================================================
inline zeptodb::sql::QueryResultSet merge_scalar_with_sql_aggs(
    const std::vector<zeptodb::sql::QueryResultSet>& results,
    const std::vector<zeptodb::sql::AggFunc>&        sql_aggs)
{
    if (results.empty()) return {};
    ...
    for (size_t ci = 0; ci < ncols && ci < sql_aggs.size(); ++ci) {
        if (sql_aggs[ci] == zeptodb::sql::AggFunc::MIN)
            row[ci] = std::numeric_limits<int64_t>::max();
        else if (sql_aggs[ci] == zeptodb::sql::AggFunc::MAX)
    ...
    for (const auto& r : results) {
        ...
        for (size_t ci = 0; ci < ncols && ci < src.size(); ++ci) {
            int64_t v = src[ci];
            zeptodb::sql::AggFunc af = (ci < sql_aggs.size())
                                    ? sql_aggs[ci]
                                    : zeptodb::sql::AggFunc::NONE;
            switch (af) {
                case zeptodb::sql::AggFunc::SUM:
                case zeptodb::sql::AggFunc::COUNT:
                case zeptodb::sql::AggFunc::VWAP: {
                    int64_t tmp;
                    if (__builtin_add_overflow(row[ci], v, &tmp)) {
                        merged.error = "integer overflow in distributed SUM/VWAP";
                        return merged;
                ...
                case zeptodb::sql::AggFunc::MIN:
                    row[ci] = std::min(row[ci], v);
                    break;
                case zeptodb::sql::AggFunc::MAX:
                    row[ci] = std::max(row[ci], v);
                    break;
                case zeptodb::sql::AggFunc::AVG:
                    row[ci] += v;
                    avg_count[ci] += 1;
                    break;
                default:
    ...
    for (size_t ci = 0; ci < ncols; ++ci) {
        zeptodb::sql::AggFunc af = (ci < sql_aggs.size())
                                ? sql_aggs[ci] : zeptodb::sql::AggFunc::NONE;
        if (af == zeptodb::sql::AggFunc::AVG && avg_count[ci] > 0) {
            int64_t s = row[ci], c = avg_count[ci];
            row[ci] = (s >= 0) ? (s + c / 2) / c : (s - c / 2) / c;
        }
    }
...
// ============================================================================
inline zeptodb::sql::QueryResultSet merge_group_by_results(
    const std::vector<zeptodb::sql::QueryResultSet>& results,
    const std::vector<bool>&                      col_is_key,
    const std::vector<zeptodb::sql::AggFunc>&        col_aggs)
{
    if (results.empty()) return {};
        ...
        for (const auto& src : r.rows) {
            ...
            if (it == groups.end()) {
                ...
                for (size_t ci = 0; ci < ncols; ++ci) {
                    if (ci < col_is_key.size() && col_is_key[ci]) {
                        ...
                        zeptodb::sql::AggFunc af = ci < col_aggs.size()
                                                ? col_aggs[ci]
                                                : zeptodb::sql::AggFunc::NONE;
                        if (af == zeptodb::sql::AggFunc::MIN)
                            acc[ci] = std::numeric_limits<int64_t>::max();
                        else if (af == zeptodb::sql::AggFunc::MAX)
            ...
            for (size_t ci = 0; ci < ncols && ci < src.size(); ++ci) {
                ...
                switch (af) {
                    ...
                    case zeptodb::sql::AggFunc::VWAP: {
                        int64_t tmp;
                        if (__builtin_add_overflow(acc[ci], v, &tmp)) {
                            merged.error = "integer overflow in distributed SUM/VWAP";
                            return merged;
                        }
                        acc[ci] = tmp; break;
    ...
    for (auto& [key, acc] : groups) {
        ...
        for (size_t ci = 0; ci < ncols; ++ci) {
            zeptodb::sql::AggFunc af = ci < col_aggs.size()
                                    ? col_aggs[ci]
                                    : zeptodb::sql::AggFunc::NONE;
            if (af == zeptodb::sql::AggFunc::AVG && cnt[ci] > 0) {
