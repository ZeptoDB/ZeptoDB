...
public:
    ...
    // ----------------------------------------------------------------
    // Dual-write during migration (P0-5)
    // ----------------------------------------------------------------

    /// Mark a symbol as migrating: ticks should be written to both nodes.
    void begin_migration(SymbolId symbol, NodeId from, NodeId to) {
        std::lock_guard<std::mutex> lock(cache_mutex_);
