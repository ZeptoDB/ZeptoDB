#pragma once
// ============================================================================
// ZeptoDB: DistributedQueryScheduler — TCP RPC 기반 분산 스케줄러
// ============================================================================
...
public:
        ...
        std::vector<std::string> node_endpoints = {})
    {
        for (auto& ep : node_endpoints) {
            ...
            clients_.push_back(
                std::make_unique<zeptodb::cluster::TcpRpcClient>(host, port));
        }
    }
...
private:
    std::vector<std::unique_ptr<zeptodb::cluster::TcpRpcClient>> clients_;
};
