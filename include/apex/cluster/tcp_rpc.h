...
using SqlQueryCallback   = std::function<apex::sql::QueryResultSet(const std::string&)>;
using TickIngestCallback = std::function<bool(const apex::ingestion::TickMessage&)>;
using WalReplayCallback  = std::function<size_t(const std::vector<apex::ingestion::TickMessage>&)>;
...
public:
    ...
    bool ingest_tick(const apex::ingestion::TickMessage& msg);

    /// Ping the remote server.  true = reachable within connect_timeout_ms.
    bool ping();

    /// Send a batch of WAL entries to the replica node.
    bool replicate_wal(const std::vector<apex::ingestion::TickMessage>& batch);
