#pragma once
// ============================================================================
// APEX-DB: Apache Kafka Consumer
// ============================================================================
// Consumes market data from a Kafka topic and routes ticks into the
// APEX-DB ingestion pipeline.
//
// Single-node:   set_pipeline()  → all ticks → local ApexPipeline
...
struct KafkaConfig {
    ...
    std::string topic;
    std::string group_id         = "apex-consumer";
...
public:
    ...
    void set_pipeline(apex::core::ApexPipeline* pipeline);
    ...
    bool ingest_decoded(apex::ingestion::TickMessage msg);
    ...
    static std::optional<apex::ingestion::TickMessage>
...
private:
    ...
    apex::core::ApexPipeline* pipeline_ = nullptr;

    // Multi-node routing
    apex::cluster::NodeId local_id_ = 0;
    std::shared_ptr<apex::cluster::PartitionRouter> router_;
    std::unordered_map<apex::cluster::NodeId,
        std::shared_ptr<apex::cluster::TcpRpcClient>> remotes_;
