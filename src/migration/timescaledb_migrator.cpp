// ============================================================================
// ZeptoDB: TimescaleDB Migration Toolkit Implementation
// ============================================================================
...
std::string ZeptoToTSDBTypeMapper::zepto_to_pg(const std::string& zepto_type) {
    if (zepto_type == "BOOLEAN")   return "BOOLEAN";
    if (zepto_type == "TINYINT")   return "SMALLINT";
    if (zepto_type == "SMALLINT")  return "SMALLINT";
    if (zepto_type == "INTEGER")   return "INTEGER";
    if (zepto_type == "BIGINT")    return "BIGINT";
    if (zepto_type == "REAL")      return "REAL";
    if (zepto_type == "DOUBLE")    return "DOUBLE PRECISION";
    if (zepto_type == "VARCHAR")   return "TEXT";
    if (zepto_type == "DATE")      return "DATE";
    if (zepto_type == "TIMESTAMP") return "TIMESTAMPTZ";
    if (zepto_type == "TIME")      return "TIME";
    return "TEXT";
}

std::string ZeptoToTSDBTypeMapper::ktype_to_pg(int8_t kdb_type) {
    switch (kdb_type) {
    ...
    const std::string& time_column)
{
    ...
    for (const auto& [col_name, ktype] : kdb_columns) {
        ...
        col.name = col_name;
        col.pg_type = ZeptoToTSDBTypeMapper::ktype_to_pg(ktype);
...
std::string TimescaleDBQueryTranslator::translate(const std::string& zepto_sql) {
...
std::string TimescaleDBMigrator::generate_migration_sql() {
    ...
    ss << "-- ================================================================\n";
    ss << "-- ZeptoDB → TimescaleDB Migration Script\n";
    ss << "-- Generated by zepto-migrate\n";
    ss << "-- ================================================================\n\n";
...
bool TimescaleDBMigrator::run() {
    ...
    // Write to file
    std::string sql_file = "/tmp/zepto_timescaledb_migration.sql";
