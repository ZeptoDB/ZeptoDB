...
| 메트릭 | 타입 | 설명 |
|--------|------|-------------|
| `apex_ticks_ingested_total` | counter | 총 인제스트된 틱 |
| `apex_ticks_stored_total` | counter | 저장된 틱 |
| `apex_ticks_dropped_total` | counter | 드롭된 틱 |
| `apex_queries_executed_total` | counter | 실행된 쿼리 |
| `apex_rows_scanned_total` | counter | 스캔된 행 |
| `apex_server_up` | gauge | 서버 실행 상태 (0/1) |
| `apex_server_ready` | gauge | 준비 상태 (0/1) |
...
APEX-DB는 JSON 형식으로 구조화된 로그를 생성합니다.
