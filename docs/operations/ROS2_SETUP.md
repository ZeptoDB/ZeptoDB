...
2. Verifies `rclcpp`, `std_msgs`, `rosbag2_cpp`, `rosbag2_storage`,
...
4. Configures a focused ZeptoDB build with `-DZEPTO_USE_ROS2=ON`,
   ...
   `Ros2ConsumerTest.*`, including generated sqlite3 rosbag2 import tests and
   the live `rclcpp` publish to ZeptoDB table ingest test.
...
- The current bridge is read-only. It subscribes to scalar `std_msgs` topics
  and imports/replays scalar rosbag2 topics without publishing back into the
  ROS graph.
