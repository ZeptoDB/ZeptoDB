...
형식: `apex_` + 64자 hex 문자 (256비트 엔트로피).

```bash
curl -X POST http://localhost:8123/ \
  -H "Authorization: Bearer apex_a1b2c3d4...64자hex" \
  -d 'SELECT count(*) FROM trades'
```
...
```bash
curl http://localhost:8123/stats -H "Authorization: Bearer apex_..."
```
