...
`#include "apex/core/pipeline.h"` — 네임스페이스: `apex::core`
...
`#include "apex/sql/executor.h"` — 네임스페이스: `apex::sql`

SQL 문자열을 파싱하고 `ApexPipeline`에 대해 실행합니다.
...
`#include "apex/storage/partition_manager.h"` — 네임스페이스: `apex::storage`
...
`#include "apex/auth/cancellation_token.h"` — 네임스페이스: `apex::auth`
