...
| 인터페이스 | 설명 | 사용 사례 |
|-----------|------|-----------|
| `zeptodb` | 저수준 pybind11 C++ 바인딩 | 인프로세스, 최대 성능 |
| `zepto_py` | 고수준 Python 패키지 | Pandas/Polars/Arrow 상호운용, HTTP 클라이언트 |

---

## 목차

- [zeptodb — pybind11 바인딩](#apex--pybind11-바인딩)
  - [zeptodb.Pipeline](#zeptopipeline)
  - [zeptodb.sql.QueryExecutor](#zeptosqlqueryexecutor)
- [zepto_py.connection — HTTP 클라이언트](#zepto_pyconnection--http-클라이언트)
- [zepto_py.dataframe — 대용량 인제스트/익스포트](#zepto_pydataframe--대용량-인제스트익스포트)
- [zepto_py.arrow — Arrow / DuckDB 상호운용](#zepto_pyarrow--arrow--duckdb-상호운용)
- [zepto_py.streaming — 고처리량 인제스트](#zepto_pystreaming--고처리량-인제스트)
...
## zeptodb — pybind11 바인딩

`zeptodb` 모듈은 pybind11로 빌드된 저수준 C++ 바인딩입니다.
...
ZeptoDB 내부 컬럼 버퍼에 대한 numpy 뷰를 반환합니다 — 복사 없음, ~522ns.
...
## zepto_py.connection — HTTP 클라이언트

포트 8123에서 실행 중인 `zepto_server`에 연결합니다 (ClickHouse 호환 HTTP API).
...
# 연결
db = zeptodb.connect("localhost", 8123)
db = zeptodb.connect("localhost", 8123, api_key="zepto_<64자 hex>")  # 인증 포함
...
## zepto_py.dataframe — 대용량 인제스트/익스포트

HTTP 없이 직접 `zeptodb.Pipeline` 객체를 사용하는 컨버터입니다.
...
## zepto_py.arrow — Arrow / DuckDB 상호운용
...
|---------------|--------|--------|-------|-------|--------|------|
| **ZeptoDB (인프로세스)** | `to_pandas()` | `to_polars()` | `get_column()` | `to_arrow()` | `to_duckdb()` | — |
