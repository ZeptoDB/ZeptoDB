# ZeptoDB Devlog #009: GROUP BY 최적화 + 타임스탬프 범위 인덱스
...
이번 작업은 ZeptoDB SQL 실행 엔진의 두 가지 핵심 성능 최적화와 추가 SQL 기능 구현이다.
...
| 파일 | 변경 내용 |
|------|-----------|
| `include/zeptodb/storage/partition_manager.h` | `timestamp_range()`, `overlaps_time_range()` 추가 |
| `include/zeptodb/sql/executor.h` | `eval_where_ranged()`, `extract_time_range()`, `apply_order_by()` 선언 |
