# ZeptoDB Phase 007: SQL 파서 + HTTP API + JOIN 프레임워크
...
ZeptoDB의 핵심 레이어(E, B, A, D, C)가 완성된 이후, 실제 클라이언트가 사용할 수 있는 인터페이스를 추가했다. 이번 Phase에서는 다음 세 가지를 구현했다:

1. **SQL 파서** — 재귀 하강 파서 (no yacc/bison)
2. **쿼리 실행기** — AST → ZeptoPipeline API 변환
...
ZeptoDB 파티션은 `symbol` 컬럼이 없다. `SymbolId`는 `PartitionKey.symbol_id`로 인코딩된다.  
