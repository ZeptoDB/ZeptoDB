...
시계열 데이터베이스로서 ZeptoDB가 실제 금융 워크로드를 처리할 수 있는 핵심 기능들이다.
...
### 수정 파일
- `include/zeptodb/sql/tokenizer.h` — 새 토큰 타입 (XBAR, EMA, DELTA, RATIO, WINDOW, PLUS, MINUS)
- `src/sql/tokenizer.cpp` — 새 키워드 매핑, MINUS/PLUS 토큰 처리
- `include/zeptodb/sql/ast.h` — AggFunc, WindowFunc, WJAggFunc, SelectExpr, GroupByClause, JoinClause 확장
- `src/sql/parser.cpp` — 모든 새 구문 파싱 (xbar, ema, delta, ratio, first, last, wj_*, WINDOW JOIN)
- `include/zeptodb/execution/window_function.h` — WindowEMA, WindowDelta, WindowRatio 클래스 추가
- `include/zeptodb/execution/join_operator.h` — JoinType enum, WindowJoinOperator, JOIN_NULL 상수
- `src/execution/join_operator.cpp` — LEFT JOIN 구현, WindowJoinOperator 이진 탐색 구현
- `include/zeptodb/sql/executor.h` — exec_window_join 선언
