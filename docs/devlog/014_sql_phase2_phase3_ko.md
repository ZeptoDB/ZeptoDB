...
이 개발 로그는 APEX-DB의 SQL 서브셋을 시계열 분석 워크로드에서 ClickHouse와 실질적으로 동등한 수준으로 끌어올린 세 번의 SQL 파서 및 실행기 개선을 다룹니다.
...
| 파일 | 변경 사항 |
|------|-----------|
| `include/apex/sql/tokenizer.h` | DATE_TRUNC, NOW, EPOCH_S, EPOCH_MS, LIKE, UNION, ALL, INTERSECT, EXCEPT 토큰 추가 |
| `src/sql/tokenizer.cpp` | 새 토큰 키워드 매핑 |
| `include/apex/sql/ast.h` | `ArithExpr::Kind::FUNC` + func 필드; `Expr::Kind::LIKE` + `like_pattern`; `SelectStmt::SetOp` + `rhs` |
| `include/apex/sql/parser.h` | `parse_string_literal()` 선언 |
