...
기존 HDB는 APEX-DB 전용 바이너리 포맷(`.bin`)만 지원했다.
...
| 파일 | 역할 |
|------|------|
| `include/apex/storage/parquet_writer.h` | ParquetWriter 헤더 (Arrow/Parquet API) |
| `src/storage/parquet_writer.cpp` | Parquet 직렬화 구현 |
| `include/apex/storage/s3_sink.h` | S3Sink 헤더 (AWS SDK C++) |
| `src/storage/s3_sink.cpp` | S3 업로드 구현 |
| `include/apex/storage/flush_manager.h` | FlushConfig에 출력 형식 옵션 추가 |
...
| APEX-DB ColumnType | Arrow DataType | Parquet 물리 타입 |
|--------------------|----------------|-------------------|
| INT32              | int32()        | INT32 |
| INT64              | int64()        | INT64 |
...
```sql
-- S3에 저장된 APEX-DB HDB를 DuckDB로 직접 쿼리
INSTALL httpfs;
LOAD httpfs;
