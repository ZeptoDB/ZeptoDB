...
목표는 단순하다:  
**ZeptoDB를 여러 노드에 걸쳐 수평 확장하되, hot path에서 virtual call 오버헤드 제로.**
...
class ClusterNode {
    ...
    ZeptoPipeline    local_pipeline_; // 로컬 APEX 파이프라인
};
```
...
sizeof(ZeptoPipeline)                  = 8,389,312 bytes  // TickPlant 65K 슬롯 때문
```
...
(ZeptoPipeline이 이렇게 크다는 건 Phase A에서 설계한 RingBuffer 크기 때문)
...
*Phase E → B → A → D → C 순서로 완성해가는 ZeptoDB.*  
