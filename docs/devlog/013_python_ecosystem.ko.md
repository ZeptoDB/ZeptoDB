...
ZeptoDB의 전체 Python 에코시스템 통합을 구현했다. pandas, polars, pyarrow, numpy, duckdb 등 Python 과학 스택과의 원활한 데이터 교환을 가능하게 한다. 이를 통해 분석가가 Jupyter 노트북에서 프로토타이핑한 후 프로덕션 규모의 실시간 쿼리를 위해 ZeptoDB로 데이터를 이동하는 "연구에서 프로덕션으로" 갭을 해소했다.
...
tbl  = sess.to_arrow(symbol=1)          # ZeptoDB → pa.Table (제로카피)
conn = sess.to_duckdb(symbol=1)         # DuckDB 테이블로 등록
pl_df = sess.to_polars_zero_copy(sym=1) # ZeptoDB → polars (Arrow 경유)
```
...
- Arrow 버퍼를 통한 Polars와 ZeptoDB 간 진정한 제로카피
...
│   ├── TestArrowSchemaUtilities — zepto_schema_to_arrow (2개)
...
| 파일 | 변경 |
|------|--------|
| `zepto_py/dataframe.py` | `from_pandas()`/`from_polars()` 재작성: iterrows() → numpy 벡터화 배치 |
| `zepto_py/dataframe.py` | `from_arrow()` 함수 + `_arrow_col_to_numpy()` 널 안전 헬퍼 추가 |
| `zepto_py/arrow.py` | `ingest_arrow()`를 `from_arrow()`에 위임하도록 재작성 (벡터화) |
| `zepto_py/arrow.py` | 컬럼별 Arrow 배열 인제스트를 위한 `ingest_arrow_columnar()` 추가 |
| `zepto_py/arrow.py` | 새 벡터화 경로를 사용하도록 `ingest_record_batch()` 업데이트 |
| `src/transpiler/python_binding.cpp` | `ingest_float_batch()` 추가 — float64 C++ 배치 인제스트 |
| `zepto_py/__init__.py` | `from_arrow`, `from_polars_arrow` 익스포트 |
...
- **`pip install zeptodb`** — Linux/macOS용 사전 빌드 휠이 포함된 PyPI 패키지
- **Jupyter 통합** — 풍부한 노트북 출력을 위한 `zepto_py.display()`
