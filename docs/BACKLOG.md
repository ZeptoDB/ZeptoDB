...
> ✅ 2026-04-30: **Stateless `zepto_ingest_node` binary** (devlog 113). New `tools/zepto_ingest_node.cpp` — an ingest-only pod that accepts HTTP INSERT and forwards every tick to the right storage pod via `CoordinatorRoutingAdapter` (devlog 111). Owns zero data: self `node_id=99999` keeps the consistent-hash ring from ever picking self (Option A — zero adapter changes). DDL replicates via devlog 112. Dockerfile.bench (x86_64 + arm64) now ships the binary alongside `zepto_http_server` / `zepto_data_node`. Helm opt-in template `ingest-deployment.yaml` + `values.yaml` `ingest:` block (disabled by default; storage-node discovery is a scaffold — operators pass `--add-node` via `extraArgs` until the init container lands). 1292/1292 preserved — no new tests (routing covered by `test_coordinator_routing_adapter.cpp`). **Closes P8-I3.** Unlocks independent ingest↔storage scaling.

> ✅ 2026-04-30: **DDL replication across cluster pods** (devlog 112). `QueryCoordinator::forward_ddl_to_remotes()` fire-and-forget replicates `CREATE / DROP / ALTER TABLE` to every remote pod after the HTTP server executes the DDL locally. Uses the already-parsed `cached_ps` (devlog 091 F4) for classification — no extra parse, no string matching. Remote failures emit `ZEPTO_WARN` but never fail the client request. Known limitation: not strongly consistent — a pod that is down during DDL misses the change until the next DDL or restart; acceptable for the benchmark-harness use case that motivated the fix. 4 new `DDLReplication.*` tests (1288 → 1292). **Closes P8-DDL-replication.**
...
| ~~**P8-I3 — Stateless `zepto_ingest_node` binary**~~ ✅ (devlog 113) — `tools/zepto_ingest_node.cpp` wires `CoordinatorRoutingAdapter` + `QueryCoordinator` + peer `TcpRpcClient` pool behind an HTTP server, with no local storage. Self `node_id=99999` keeps the partition-router ring from ever picking self, so every tick is forwarded over RPC (Option A — zero adapter changes). DDL replicates automatically via devlog 112. Dockerfile.bench (x86_64 + arm64) now ships the binary alongside `zepto_http_server` / `zepto_data_node`. Helm opt-in template `ingest-deployment.yaml` + `values.yaml` `ingest:` block (disabled by default). Baseline 1292/1292 preserved — no new tests; routing correctness already covered by `test_coordinator_routing_adapter.cpp`. | Independent ingest scaling | — |
