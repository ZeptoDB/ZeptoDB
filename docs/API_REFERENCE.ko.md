...
| [docs/api/PYTHON_REFERENCE_ko.md](api/PYTHON_REFERENCE_ko.md) | `zeptodb` pybind11 바인딩, `zepto_py` 패키지 — connection, dataframe, arrow, streaming |
| [docs/api/CPP_REFERENCE_ko.md](api/CPP_REFERENCE_ko.md) | `ZeptoPipeline`, `QueryExecutor`, `PartitionManager`, `TickMessage`, `CancellationToken` |
...
```python
import zeptodb
from zepto_py import from_polars, to_pandas
...
curl -X POST http://localhost:8123/ \
  -H "Authorization: Bearer zepto_<키>" \
  -d 'SELECT vwap(price, volume) FROM trades WHERE symbol = 1'

# 통계
curl http://localhost:8123/stats -H "Authorization: Bearer zepto_<키>"
