...
Run ZeptoDB's UCX transport on AWS EFA (Elastic Fabric Adapter) to measure RDMA one-sided read/write performance. Quantify the latency/throughput difference compared to TCP RPC and verify the real-world benefits for HFT workloads.

### TCP vs RDMA — Why We Need to Measure
...
Select the smallest and cheapest instance that supports both EFA and RDMA read/write.
...
**Total cost: ~$1.12/hr (Spot) → 2-hour bench = ~$2.25**

**Why `m7a.4xlarge`:**
- Nitro v4 → supports both RDMA read and write (lowest cost among EFA-capable instances)
- AMD EPYC (Zen 4) — AVX-512 for Highway SIMD
- 16 vCPU, 64 GB RAM — sufficient for a ZeptoDB 3-node cluster
- m7a is a general-purpose family with abundant Spot inventory and low interruption probability
- Coordinator does not require a separate instance; one of the Data Nodes serves dual duty

**Alternative options:**

| Option | Instance | RDMA R/W | Network | Spot/hr (×4) | Total cost (2h) | Notes |
|--------|----------|----------|---------|-------------|-------------|------|
| **A (recommended)** | `m7a.4xlarge` | ✅ R+W | 6.25 Gbps | ~$1.12 | **~$2.25** | Lowest cost RDMA R/W |
| B | `hpc7g.4xlarge` | Read only | 200 Gbps | $6.72 (OD) | ~$13.44 | High bandwidth, ARM, Spot unavailable |
| C | `c5n.9xlarge` | ❌ SRD only | 50 Gbps | ~$2.33 | ~$4.66 | Not RDMA, EFA SRD only |
| D | `m6i.32xlarge` | ✅ R+W | 50 Gbps | ~$5.90 | ~$11.80 | When high bandwidth is needed |

> **Note:** The `c5n` series uses Nitro v3, which supports EFA but not RDMA read/write.
> To test true one-sided RDMA (ucp_put/ucp_get), Nitro v4+ is required.
...
UCX one-sided write latency — direct measurement of the ZeptoDB transport layer.
...
Measures the path of reading column data directly from a remote node via RDMA GET.
...
ASOF JOIN has high network dependency since it needs to read data from both tables.
...
Ingest at maximum load for 30 seconds while simultaneously executing queries. Measures the CPU savings from RDMA.
...
|----------|-----------|----------|-----------|
| **RDMA R/W bench (recommended)** | UCX/EFA | 4× `m7a.4xlarge` Spot | **~$2.25** |
| RDMA Read + high bandwidth | UCX/EFA | 4× `hpc7g.4xlarge` OD | ~$13.44 |
| EFA SRD (not RDMA) | UCX/SRD | 4× `c5n.9xlarge` Spot | ~$4.66 |
| EKS TCP bench | TCP RPC | EKS r7i.2xlarge | ~$12 |
| Dev/CI | SharedMem | Local | $0 |
