...
> ZeptoDB와 비교하기 위한 kdb+ 및 경쟁 DB 벤치마크 데이터 모음
> 마지막 업데이트: 2026-03-22
...
## 2. kdb+ vs ZeptoDB 직접 비교

### 인제스션 비교

| 메트릭 | kdb+ tickerplant | ZeptoDB | 비고 |
|---|---|---|---|
...
| 연산 | kdb+ (추정) | ZeptoDB Scalar | ZeptoDB SIMD | 갭 |
|---|---|---|---|---|
...
2. **ZeptoDB filter는 SelectionVector 기록 오버헤드** — bitmask 방식으로 개선 예정
3. **ZeptoDB sum은 이미 auto-vectorize** — 추가 개선 여지 적음 (메모리 대역폭 bound)
4. **ZeptoDB 파티셔닝 오버헤드** — 단일 파티션 내 직접 스캔이 아닌 인덱스 탐색 비용
...
**ClickHouse 특성:**
- 벡터화 실행 엔진 (ZeptoDB Layer 3과 유사한 아키텍처)
- MergeTree 스토리지 (ZeptoDB DMMT에 영감)
...
- 틱 단위 스트리밍 인제스션
- 인메모리 성능 (kdb+, ZeptoDB 도메인)

### ZeptoDB vs ClickHouse 포지셔닝

| 차원 | ClickHouse | ZeptoDB |
|---|---|---|
| 타겟 | 범용 OLAP | HFT 실시간 |
...
## 4. 결론 및 ZeptoDB 목표 수치

### 최종 목표 (Phase B 최적화 후)

| 메트릭 | kdb+ (최신 추정) | ZeptoDB 목표 | 전략 |
|---|---|---|---|
