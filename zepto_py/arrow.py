"""
ZeptoDB Arrow Integration — zero-copy data exchange via Apache Arrow.
...
# ============================================================================
# ZeptoDB ↔ Arrow Schema Mapping
# ============================================================================

# Maps ZeptoDB SQL types to Arrow types
ZEPTO_TO_ARROW = {
...
def zepto_type_to_arrow(zepto_type: str) -> Any:
    """Convert ZeptoDB type string to pyarrow type."""
    if not HAS_PYARROW:
        raise ImportError("pyarrow is required: pip install pyarrow")
    factory = ZEPTO_TO_ARROW.get(zepto_type.upper())
...
class ArrowSession:
    """
    ZeptoDB session with Apache Arrow integration.

    Provides zero-copy data exchange between ZeptoDB and:
    ...
    def to_arrow(
        ...
        """
        Export ZeptoDB data as an Arrow Table.
    ...
    def to_duckdb(
        self,
        symbol: int,
        table_name: str = "zepto_data",
        conn: Any = None,
    ) -> Any:
        """
        Register ZeptoDB data as a DuckDB table.
    ...
    def infer_schema(
        ...
        """Infer Arrow schema from ZeptoDB column data."""
        table = self.to_arrow(symbol=symbol)
        return table.schema

    def zepto_schema_to_arrow(
        self,
        zepto_schema: List[tuple],
    ) -> "pa.Schema":
        """
        Convert ZeptoDB schema description to Arrow schema.

        Parameters
        ----------
        zepto_schema : list of (name, zepto_type_str)
        ...
        """
        fields = []
        for name, zepto_type in zepto_schema:
            fields.append(pa.field(name, zepto_type_to_arrow(zepto_type)))
